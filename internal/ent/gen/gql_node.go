...
func (c *Client) noder(ctx context.Context, table string, id string) (Noder, error) {
	switch table {
	case googleauth.Table:
		query := c.GoogleAuth.Query().
			Where(googleauth.ID(id))
		query, err := query.CollectFields(ctx, "GoogleAuth")
		if err != nil {
			return nil, err
		}
