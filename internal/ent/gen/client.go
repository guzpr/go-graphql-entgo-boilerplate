...
import (
	...
	"github.com/sekalahita/epirus/internal/ent/gen/migrate"
...
func (c *GoogleAuthClient) UpdateOneID(id string) *GoogleAuthUpdateOne {
...
func (c *GoogleAuthClient) DeleteOneID(id string) *GoogleAuthDeleteOne {
...
func (c *GoogleAuthClient) Get(ctx context.Context, id string) (*GoogleAuth, error) {
	return c.Query().Where(googleauth.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *GoogleAuthClient) GetX(ctx context.Context, id string) *GoogleAuth {
...
func (c *GoogleAuthClient) Hooks() []Hook {
	hooks := c.hooks.GoogleAuth
	return append(hooks[:len(hooks):len(hooks)], googleauth.Hooks[:]...)
}

// Interceptors returns the client interceptors.
func (c *GoogleAuthClient) Interceptors() []Interceptor {
	inters := c.inters.GoogleAuth
	return append(inters[:len(inters):len(inters)], googleauth.Interceptors[:]...)
}
...
func (c *GoogleAuthClient) mutate(ctx context.Context, m *GoogleAuthMutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("gen: unknown GoogleAuth mutation op: %q", m.Op())
	}
}
...
// QueryGoogleAuth queries the google_auth edge of a User.
func (c *UserClient) QueryGoogleAuth(u *User) *GoogleAuthQuery {
	query := (&GoogleAuthClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
