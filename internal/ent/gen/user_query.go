...
type UserQuery struct {
	...
	withGoogleAuth *GoogleAuthQuery
...
// QueryGoogleAuth chains the current query on the "google_auth" edge.
func (uq *UserQuery) QueryGoogleAuth() *GoogleAuthQuery {
	query := (&GoogleAuthClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.To(googleauth.Table, googleauth.FieldID),
			sqlgraph.Edge(sqlgraph.O2O, false, user.GoogleAuthTable, user.GoogleAuthColumn),
		)
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withGoogleAuth: uq.withGoogleAuth.Clone(),
...
// WithGoogleAuth tells the query-builder to eager-load the nodes that are connected to
// the "google_auth" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithGoogleAuth(opts ...func(*GoogleAuthQuery)) *UserQuery {
	query := (&GoogleAuthClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withGoogleAuth = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withGoogleAuth; query != nil {
		if err := uq.loadGoogleAuth(ctx, query, nodes, nil,
			func(n *User, e *GoogleAuth) { n.Edges.GoogleAuth = e }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadGoogleAuth(ctx context.Context, query *GoogleAuthQuery, nodes []*User, init func(*User), assign func(*User, *GoogleAuth)) error {
	...
	query.Where(predicate.GoogleAuth(func(s *sql.Selector) {
		s.Where(sql.InValues(user.GoogleAuthColumn, fks...))
	}))
